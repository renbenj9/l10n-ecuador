...
class AccountEdiFormat(models.Model):
    ...
    def _check_move_configuration(self, document):
        ...
        if document.country_code == "EC":
            ...
            #Retencion Venta
            if document_type == "withholding" and document.move_type == "out_invoice":
                if not company.l10n_ec_withholding_version:
                    errors.append(
                        _(
                            "You must set XML Version for Withholding into company %s",
                            company.display_name,
                        )
                    )    
